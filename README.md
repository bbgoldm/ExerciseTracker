...
Hints
The REST API for this assignment is very similar to the REST API for movies implemented in Module 8. Implementing that REST API will help you implement the REST API for this assignment.
The React app for this assignment is very similar to the movie-ui React app implemented in Module 9. Follow along with the videos in that exploration to code the movie-ui React app and that will help you implement the React app for this assignment.
What to Turn In?
You will submit two zip files, one for the React app and the other for the REST API.
The REST API must run on port 3000.
For the React app, submit the development build. When we start the React app, the server must listen on port 8000.
Make sure to add a .env file with PORT=8000 to your React app and add the proxy property to the React app's package.json file
...
If your assignment uses an instance of MongoDB that is not on the localhost or it uses a port other than 27017, then add a comment with your Canvas submission specifying the MongoDB URL you are using, along with the name(s) of the file(s) in your submitted code in which the MongoDB URL is being used.
Video of a Sample Solution
Here is a video of testing a sample solution. Here is the file  Download the filethat was used to send requests to the REST API.


Grading Criteria
This assignment is worth 21% of your final grade.
...
1. Instead of String, use the Date datatype for storing the date value. For a discussion of dates in Mongoose see this documentation (Links to an external site.).

2. Deploy your app on the platform of a cloud vendor.

3. Secure your app by adding login functionality. Instead of rolling your own authentication solution, we recommend that you use a 3rd party such as Auth0 (Links to an external site.) for this.
